...
var operationMap = {
  ...
  Task: {
    'POST /{organizationKey}/tasks/{taskId}/complete': {
      name: 'completeTask',
      replaceParameters: {
        'array of FormInstanceFields': {
          paramType: 'body',
          name: 'formInstanceFields',
          dataType: 'array',
          items: {
...
function getOperationArguments(interfaceName, apiPath, op) {
  var map = operationMap[interfaceName] || {};
  var opId = util.format(op.method, apiPath);

  var parameters = op.parameters;
  if (!parameters) return parameters;

  if (map[opId] && map[opId].replaceParameters) {
    var replaceParameters = map[opId].replaceParameters;
    _.forEach(replaceParameters, function(parm, key) {
      var parmIndex = _.findIndex(parameters, {
        name: key
      });

      if (parmIndex > -1) {
        parameters[parmIndex] = _.extend(parameters[parmIndex], parm);
      }
    });
  }
