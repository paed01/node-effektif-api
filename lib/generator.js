...
var operationMap = {
  User: {
    createRegistrationActivate: 'activateRegistration'
  },
  Process: {
    createProcesses: 'createProcess'
  }
};
...
var getOperationName = function(interfaceName, apiPath, method) {
  ...
  if (map[opName]) {
    debug(apiPath, method, opName, 'is renamed to', map[opName]);
    opName = map[opName];
  }
...
var mapArrayType = function(arrayProp, apiDef, parentTypeStack) {
  ...
  var itemsType = mapBodyType(refType, apiDef, null, parentTypeStack);

  model = model.items(itemsType).meta({
    originalType: refType
  });
  return model;
};
...
var ioptionsSchema = Joi.object().keys({
  ...
  onUnauthorized: Joi.func().optional().description('Excecuted when an unauthorized call was made').notes(['Receives operation arguments and callback']),
      ...
      var operation = function() {
        ...
        if (validationError) {
          ...
          if (/authorization.*required/i.test(validationError.message)) {
            // Make login request
            return self._onUnauthorized(operationArgs, function(authErr, authResp, authBody) {
        ...
        function innerCallback(err, resp, body) {
          ...
          if (resp.statusCode === 401) {
            ...
            if (requiresAuthorization && self.options.credentials) {
              // Make login request
              return self._onUnauthorized(operationArgs, function(authErr, authResp, authBody) {
                if (authErr) return callback(authErr, authResp, authBody);
