...
var mapOutput = function(outputType, apiDef) {
  ...
  var mSchema = Joi.compile(outputSchema).label(outputType).meta({'originalType': outputType});
  return mSchema;
};
...
var ioptionsSchema = Joi.object().keys({
  ...
  onUnauthorized: Joi.func().optional().description('Excecuted when an unauthorized call was made'),
  ...
  apiDef.apis.forEach(function(api) {
    api.operations.forEach(function(op) {
      ...
      if (operationMap[opName]) {
        debug(api.path, op.method, opName, 'is renamed to', operationMap[opName]);
        opName = operationMap[opName];
      }
